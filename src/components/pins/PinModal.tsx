...
const PinModal: React.FC<PinModalProps> = ({ id, onClose }) => {
  ...
  if (!pin || !newComment.trim()) return; // nada si está vacío

  try {
    await addComment(pin.id, comment); // llamar servicio
    setComments([...comments, { id: Date.now(), text: newComment }]); // actualizar solo si es exitoso
          ...
          <div className="flex-1 flex flex-col">
            <div className="w-full h-40 md:h-56 border border-gray-500 rounded-t-lg overflow-y-auto p-2">
              {comments.length === 0 ? (
                ...
                comments.map((c) => (
                  <p key={c.id} className="text-white text-sm mb-1">{c.text}</p>
                ))
              )}
            </div>
